...
function blip:didMount()
	self.renderLoop = game:GetService("RunService").RenderStepped:connect(function()
		...
		if BlipController:getDistanceFromPlayer(self.props.Object.Position) <= Settings["Technical"]["maxBlipDistance"] then
			local Pos = BlipController:getUIPosition(Vector3.new(self.props.Object.Position.X, 0, self.props.Object.Position.Z) / Settings["Divide"], map.AbsoluteSize)
			local Rotation = 0
			...
			if TagInfo.rotate == true then
				...
				if Settings["Technical"]["rotation"] then
					Rotation = heading - math.deg(getAngleAboutYAxis(self.props.Object.CFrame)) + 180
				else
					Rotation = -math.deg(getAngleAboutYAxis(self.props.Object.CFrame))
				end
			end
